...
func fetchCacheorRemote(currentURL string, cache *go_cache.Cache[[]byte]) (*TSL, []byte, error) {
	...
	if err != nil || len(bodyBytes) == 0 {
		bodyBytes, fetchErr := FetchTSLBytes(currentURL)
		if fetchErr != nil {
		...
		if unmarshError != nil {
			return nil, nil, fmt.Errorf("unmarshal %s: %w", currentURL, unmarshError)
		}
	...
	if err != nil {
		return nil, nil, fmt.Errorf("unmarshal %s: %w", currentURL, err)
	}
	return tsl, bodyBytes, nil
}
